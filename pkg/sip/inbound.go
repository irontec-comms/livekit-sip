...
func modifySDPDirection(sdpData []byte, direction string) ([]byte, error) {
	...
	// Parse the SDP using pion/sdp
	sessionDesc := &pionsdp.SessionDescription{}
	if err := sessionDesc.Unmarshal(sdpData); err != nil {
		return nil, fmt.Errorf("failed to parse SDP: %w", err)
	}

	// Modify direction attributes in each media description
	for _, mediaDesc := range sessionDesc.MediaDescriptions {
		if mediaDesc == nil {
			continue
		}
	...
	sessionDesc.Origin.SessionVersion++

	// Marshal back to bytes
	modifiedSDP, err := sessionDesc.Marshal()
	if err != nil {
